...
- リスト表示とギャラリー表示を切り替え、ギャラリーの列数を調整する
...
類似画像の確認はMac内だけで行われます。アプリが画像を自動削除したり、リストから勝手に除外したりすることはありません。
...
配布用DMGはDeveloper IDで署名し、Appleの公証を通過しています。利用するためにXcodeやターミナルは必要ありません。

## インストール

1. [最新のRelease](https://github.com/hirakke/FileRenamer/releases/latest)から `FileRenamer-*.dmg` をダウンロードします。
2. DMGを開きます。
...
- **ローカル処理**：ファイル、画像、ファイル名、利用状況を開発者へ送信しません
...
削除されません。比較候補を表示するだけで、残す画像は利用者が判断できます。検出自体を「FileRenamer」メニューの「設定…」から無効にすることもできます。
...
- [過去のReleaseと更新履歴](https://github.com/hirakke/FileRenamer/releases)
